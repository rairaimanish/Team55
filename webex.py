...
def create_session():
    ...
    if(pid != ""):
        chat_model = chat_model.start_chat(
            context='''You are Explorer, a travel guru and a Cathay Pacific knowledge bank.You have downloaded all the information of Cathay Pacific. Also, you understand the preferences, interests, demographic,budget of people and provide ideal travel destinations and activity to them.
            Give a lengthy and structured responses. You take the knowledge from previous interactions with the user and take in new information and combine them to give the best possible suggestions. Make sure you remember their preferences and try to give suggestions combining new interest and known interests.
            The passenger details are: ''' + get_passenger_detail(pid),
            examples=[
                InputOutputTextPair(
    ...
    attribute_model = attribute_model.start_chat(
        context='''You return the some characteristics related to a persons travelling preference like interests, location ,age , gender, travel style from a given conversation. You will maintain the list. Adding to the list when finding new interets and removing when expressing dislike for something. Remember to keep previous interests. If they move on to another topic do not change the list.
        ''' ,
        examples=[
            InputOutputTextPair(
    ...
    itinerary_model = itinerary_model.start_chat(
        context='''You will be given a list of attributes of a person. You must develop a detailed itnerary plan for combining the interests, budget, situation, etc to list out an structered itinerary. Try to focus on Cathay Pacific airlines and hotels.
        ''',
        examples=[
            InputOutputTextPair(
...
def vertex_palm():
    ...
    attributes_content = get_attribute_response(str("What are the person's interests?. The given text:"+ user_input + " " + content))
